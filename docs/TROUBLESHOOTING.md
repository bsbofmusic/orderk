...
| Jianling config looks right but live reflection is not called | A false/off override is set, the LLM chain is empty, or the key-env pointer is unresolved | Check `ORDERK_JIANLING_LLM_ENABLED[_PROFILE]` is not `0/false/off`; ensure `ORDERK_SWORD_LLM_API_KEY_ENV` points to the real key env, not a raw secret; reproduce the systemd user-manager env when debugging scheduled runs | `orderk jianling chat-smoke` and latest receipt show `provider_status=called_live` |
